<template>
    ...
    <v-layout class="headline mb-3" style="font-weight:bold;font-style:italic;background:lightgrey;font-size:3rem !important; height:5rem">
      <v-flex style="margin:auto"><router-link :to="'noticePage'">공지사항</router-link></v-flex>
    </v-layout>
    </v-col>
        <v-row>
            <v-col
        ...
        <div v-if="loaded">
        <div class="text-center" v-for="i in alarams.length > 3 ? 3: alarams.length" :key="i.key" style="padding:0">
            <v-sheet color="orange" v-if="alarams[i-1].ncheck">
                <v-row >
                    <v-col
                    cols="1">
                    {{alarams[i-1].noticeId}}
                    </v-col>
                    <v-col
                    cols="3">
